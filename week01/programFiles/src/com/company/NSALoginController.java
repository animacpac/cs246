...
public final class NSALoginController {
	...
	*/
	public static void hashUserPassword(User user) throws Exception {
		//Check if the password is valid (more than 8 characters and contains at least one digit)
		if ( user.getPassword().length() < 8 | !containsDigit(user.getPassword()) ) {
			throw new WeakPasswordException("Error: Password must be at least 8 digits long " +
					"and contain at least one digit.%n");
		}
